...
/**
 * Class to implement Priority Queue The implementation is using an array and
 * visualizing heap
 *
 */
public class PriorityQueueImplementation implements PriorityQueue {
	...
	public PriorityQueueImplementation(int capacity) {
		...
		this.storage = new Bowler[this.capacity];
		this.size = 0;
	}
	...
	private void swap(int index1, int index2) {
		Bowler temp = this.storage[index1];
	...
	private void shiftUp(int index) {
		...
		while (index > 0
				&& this.storage[index].getDeliveriesRemained() > this.storage[parentIndex]
						.getDeliveriesRemained()) {
	...
	private void shiftDown(int index) {
		...
		while (index < this.size) {
			...
			if (rightChildIndex < this.size) {
				if (this.storage[leftChildIndex].getDeliveriesRemained() >= this.storage[rightChildIndex]
						.getDeliveriesRemained()) {
			...
			if (this.storage[index].getDeliveriesRemained() < this.storage[shiftIndex]
					.getDeliveriesRemained()) {
