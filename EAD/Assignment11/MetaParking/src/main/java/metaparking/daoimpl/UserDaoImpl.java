...
public class UserDaoImpl implements UserDao {
	...
	public User getUserById(int empId) {
		...
		Query<User> query = session.createQuery("from User where empId = :empId", User.class);
	...
	public int getUserId(String emailId) {
		Session session = entityManager.unwrap(Session.class);
		Query<Integer> query = session.createQuery("Select empId from User where emailId = :emailId", Integer.class);
	...
	/**
	 * Function to get list of all friends working in same organization
	 * @param empId
	...
	public List<User> getFriends(int empId) {
		...
		Query<User> query = session.createQuery("from User where orgName = :orgName and empId <> :empId", User.class);
		query.setParameter("orgName", user.getOrgName());
